...
router.post("/", async (req, res) => {
  const { id_bus_stop, id_user, label } = req.body;
  let favorite_bus_stop = null;

  try {
    favorite_bus_stop = await createFavoriteBusStop(id_bus_stop, id_user, label);
...
router.get("/:id_favorite_bus_stop", async (req, res) => {
...
router.put("/:id_favorite_bus_stop", async (req, res) => {
    const { id_favorite_bus_stop } = req.params;
    let favorite_bus_stop = null;
...
router.delete("/:id_favorite_bus_stop", async (req, res) => {
    const { id_favorite_bus_stop } = req.params;
    let favorite_bus_stop = null;
